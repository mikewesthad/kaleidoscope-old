...
The kaleidoscope works by taking an image (preferably high resolution) and treating
it as if it were a [pumpkin] pie.  We can take a 'slice' of the image and draw it to the
display.  We can then rotate and draw a flipped version of that slice and repeat that process
until we've rotated all the way around the circle. Then, the particular 'slice' we are sampling
from the image can be changed over time to create an animation.
